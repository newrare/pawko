...
function touchEvent(type, { x = 0, y = 0, id = 1, target = window } = {}) {
  ...
  Object.defineProperty(event, 'touches', { get: () => (type === 'touchend' ? [] : [touch]) });
...
describe('SwipeDetector', () => {
  ...
  it('fires once during touchmove when threshold is crossed', () => {
    ...
    target.dispatchEvent(touchEvent('touchmove', { x: SWIPE_THRESHOLD + 5, y: 0 }));
  ...
  it('does not fire below threshold', () => {
    target.dispatchEvent(touchEvent('touchstart', { x: 0, y: 0 }));
    target.dispatchEvent(touchEvent('touchmove', { x: SWIPE_THRESHOLD - 1, y: 0 }));
  ...
  it('emits one direction per gesture', () => {
    target.dispatchEvent(touchEvent('touchstart', { x: 0, y: 0 }));
    target.dispatchEvent(touchEvent('touchmove', { x: SWIPE_THRESHOLD + 5, y: 0 }));
    target.dispatchEvent(touchEvent('touchmove', { x: SWIPE_THRESHOLD + 50, y: 0 }));
    target.dispatchEvent(touchEvent('touchmove', { x: 0, y: SWIPE_THRESHOLD + 5 }));
    expect(onDirection).toHaveBeenCalledTimes(1);
  });

  it('falls back to touchend when move was throttled', () => {
    target.dispatchEvent(touchEvent('touchstart', { x: 0, y: 0 }));
    target.dispatchEvent(touchEvent('touchend', { x: 0, y: SWIPE_THRESHOLD + 10 }));
    expect(onDirection).toHaveBeenCalledTimes(1);
  ...
  it('shouldIgnore prevents firing on UI elements', () => {
    ...
    target.dispatchEvent(touchEvent('touchstart', { x: 0, y: 0, target: button }));
    target.dispatchEvent(touchEvent('touchmove', { x: SWIPE_THRESHOLD + 10, y: 0 }));
    expect(onDirection).not.toHaveBeenCalled();
  });

  it('chooses the dominant axis', () => {
    target.dispatchEvent(touchEvent('touchstart', { x: 0, y: 0 }));
    target.dispatchEvent(touchEvent('touchmove', { x: 5, y: -(SWIPE_THRESHOLD + 5) }));
    expect(onDirection).toHaveBeenCalledTimes(1);
