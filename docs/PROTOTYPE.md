...
| **Peg** (a.k.a. **nail**) | A small static circular obstacle that bounces balls. Visually grey/metallic.                                  | `gt-proto-peg`          |
...
| **Camera**          | The `translateY` value applied to the **stack**. Increases as new layers are added so only the top `VISIBLE_LAYERS` stay in view. | `stackOffset` |
